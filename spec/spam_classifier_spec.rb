require 'spec_helper'
require File.expand_path(File.dirname(__FILE__) + '/spam_test')
...
describe SpamClassifier do
  ...
  describe 'SpamClassifierProbabilities#spam_ham_ratio' do
    it "should be smaller for a smaller number of spammy words" do
      SpamTest.new('this dirty test').spam_ham_ratio.should > SpamTest.new('this test').spam_ham_ratio
    end
  end

  describe "#spam_ham_ratio" do
    it "should add unknown words to the dictionary before classification" do
      SpamTest.new('newword needs to pass heuristics').classify
      Words['newword'][:spam].should == 0
      Words['newword'][:spam].should == 0
    end

    it "considers 'test goes words' ham" do
      SpamTest.new('test goes words').spam_ham_ratio.should < 1
    end

    it "considers 'rid goes dirty' spam" do
      SpamTest.new('rid goes dirty').spam_ham_ratio.should >= SpamClassifier::SPAM_THRESHOLD
    end

    it "doesn't know whether 'zero goes rid' is spam or not" do
      SpamTest.new('zero goes rid').spam_ham_ratio.between?(1, SpamClassifier::SPAM_THRESHOLD).should be_true
    end

    it "thinks of 'test boss@offshore.com' as more spam than just 'test'" do
      SpamTest.new('test boss@offshore.com').spam_ham_ratio.
        should > SpamTest.new('test').spam_ham_ratio
    end

    it "thinks of 'test www.offshore.com' as more spam than just 'test'" do
      SpamTest.new('test www.offshore.com').spam_ham_ratio.
        should > SpamTest.new('test').spam_ham_ratio
    end

    it "thinks of 'test www.offshore.com' as more spam if said by someone with a standard username" do
      standard_username_ratio = SpamTest.new('test www.offshore.com').spam_ham_ratio
      custom_username_ratio = SpamTest.new('test www.offshore.com', User.create(:username => 'mybandname')).spam_ham_ratio
      standard_username_ratio.should > custom_username_ratio
    end

    it "will tolerate urls coming from known sites" do
      SpamTest.new('test www.offshore.com').spam_ham_ratio.should >
      SpamTest.new('test www.soundcloud.com').spam_ham_ratio
    end

    it "will tolerate someone who uploaded an avatar" do
      no_avatar_ratio = SpamTest.new('test').spam_ham_ratio
      avatared_user = User.new
      avatared_user.avatars << Avatar.create
      avatar_ratio = SpamTest.new('test', avatared_user).spam_ham_ratio
      no_avatar_ratio.should > avatar_ratio
    end

    it "should tolerate someone who uploaded a track" do
      no_tracks_ratio = SpamTest.new('test').spam_ham_ratio
      someone_with_track = User.new
      someone_with_track.tracks << Track.create(:bpm => 12, :permalink => 'iikyoku', :title => 'tit', :description => 'descr')
      tracks_ratio = SpamTest.new('test', someone_with_track).spam_ham_ratio
      no_tracks_ratio.should > tracks_ratio
    end

    it "should say DUNNO if it doesnt have neither :spam nor :ham score for a message" do
      SpamTest.new('zero newword heuristicspass').spam_ham_ratio.between?(1, SpamClassifier::SPAM_THRESHOLD).should be_true
    end

    it "should say IS_SPAM if it has spam score for a message and doesn't have ham score for it" do
      a = SpamTest.new('nosuchword nowordsuch heuristicspass')
    ...
    it "should say IS_HAM if it has ham score for a message and doesn't have spam score for it" do
      a = SpamTest.new('suchwordno nowordsuch heuristicspasss')
      a.category_probability(:spam).should == 0.0
      a.category_probability(:ham).should == 0.0
      a.spam_ham_ratio.between?(1, SpamClassifier::SPAM_THRESHOLD).should be_true
      a.classify_as!(:ham)
  ...
  describe "#classify_as!" do
    it "should increase the index counts of the classified words" do
      lambda {
        SpamTest.new('zero').classify_as!(:ham)
  ...
  describe "#pass_ham_heuristics?" do
    it "shouldn't deal with comments having no url/email and less than LOWER_WORDS_LIMIT_FOR_COMMENTS words" do
      SpamTest.new('that seems quite alright', User.new, Comment).pass_ham_heuristics?.should_not be_true
    end
    it "should deal with comments having url/email independently of their word-count" do
      SpamTest.new('bye comment bad@wrong.com', User.new, Comment).pass_ham_heuristics?.should be_true
    end
    it "should deal with comments having more than LOWER_WORDS_LIMIT_FOR_COMMENTS words" do
      SpamTest.new_with_random_text(SpamClassifier::LOWER_WORDS_LIMIT_FOR_COMMENTS+1).pass_ham_heuristics?.should be_true
    end
    it "shouldn't deal with Posts having no url/email and less than LOWER_WORDS_LIMIT_FOR_POSTS words" do
      SpamTest.new('That is an example of a post that should not be dealt with by us', User.new, Post).pass_ham_heuristics?.should_not be_true
    end
    it "should deal with Posts having a url/email independently of their word-count" do
      SpamTest.new('bye post bad@wrong.com', User.new, Post).pass_ham_heuristics?.should be_true
    end
    it "should throw ArgumentError if given unexpected @spammable_class argument" do
      lambda{
        SpamTest.new('anything', User.new, 'Posttt').pass_ham_heuristics?
      }.should raise_error(ArgumentError)
    end

    it "should say DUNNO for items, which have words but none of these words has been seen by the filter" do
      SpamTest.new_with_random_text(SpamClassifier::LOWER_WORDS_LIMIT_FOR_COMMENTS+1, 10).classify.should == 0
    end
  end
