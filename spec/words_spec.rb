...
describe Words do
  ...
  describe "#[]()" do
    ...
    it "should first look in the cache and take the value from there if it exists" do
      ...
      Words.find_by_key('word::word').should == nil
      Words['word'].should_not == nil
    end
    it "should take words from data store if not found in the cache" do
      Words.fetch_all([ 'word' ])
      Words.create!('word::phrase')
      Words.find_by_key('word::word').should == nil
      Words.find_by_key('word::phrase').should_not == nil
      Words['phrase'].should == Words.find_by_key('word::phrase')
    end
    it "should create new object if no key found in the datastore and add to the cache" do
      Words['nonexisting'].should.eql? Words.class_eval{ new('word::nonexisting') }
      Words.find_by_key('word::nonexisting').should == nil
    end
  end
  ...
  describe "#increment_many" do
    ...
    it "should reload the cache before incrementing" do
      ...
      Words.class_variable_get("@@cache")['word::bar'].should_not == nil
      Words.increment_many([ 'foo', 'newbar' ], :spam)
      Words.class_variable_get("@@cache")['word::bar'].should == nil
    end
    it "should increment the values in the cache" do
      Words.class_variable_get("@@cache")['word::bar'].should == nil
      Words.increment_many([ 'foo', 'bar' ], :spam)
      Words.class_variable_get("@@cache")['word::bar'][:spam].should == 1.0
    end
    it "increments the spam/ham count for a given key to the data source" do
      Words.increment_many([ 'foo', 'bar' ], :spam)
      Words.write!
      Words.find_by_key('word::foo')[:spam].should == 1
      Words.find_by_key('word::foo')[:ham].should == 0
      ...
      Words.find_by_key('word::foo')[:spam].should == 1
      Words.write!
      Words.find_by_key('word::foo')[:spam].should == 2
      ...
      Words.find_by_key('word::foo')[:ham].should == 0
      Words.write!
      Words.find_by_key('word::foo')[:ham].should == 1
    end
  end
